...
class GetState extends StatefulWidget {
  ...
  const GetState({Key? key, required this.countryShortName}) : super(key: key);
...
class _GetStateState extends State<GetState> {
  ...
  void initState() {
    ...
    Future.delayed(Duration.zero).then((value) async => _list=await ApiResponse().getStates(countryShortname: widget.countryShortName)).then((value) =>      setState(() {
      _statesList=_list;
    }));
  }
  ...
  search(string){
    ...
    if(string!=null&&string!=''){
      _list=_statesList?.where((element) => element.toString().toLowerCase().startsWith(string.toString().toLowerCase())).toList();
    }else{
      _list=_statesList;
    }    });
  }
